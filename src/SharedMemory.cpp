...
namespace cxx
{
    ...
    bool SharedMemory::CreateFromFile(HANDLE hFile)
    {
        if (hFile != nullptr)
        {
            ...
            if (this->FromFile)
            {
                CloseHandle(this->FromFile);
            }
            if ((hFileMap = CreateFileMapping( this->FromFile = hFile, nullptr, PAGE_READWRITE, 0, Size, MapName.c_str())) == nullptr)
            {
            ...
            if ((pData = mmap(nullptr, Size, PROT_READ | PROT_WRITE, MAP_FILE | MAP_SHARED, this->FromFile = hFile, 0)) == MAP_FAILED)
            {
                if (Debug) tcout << _T("\nCould Not Map View Of File.\n");
                return false;
            }
